...
const SEPA_START_DATE = process.env.SEPA_START_DATE;

function parseSepaStartDateOrThrow() {
    if (!SEPA_START_DATE || !String(SEPA_START_DATE).trim()) {
        throw new Error("Missing SEPA_START_DATE (expected format YYYY-MM-DD)");
    }
    const iso = `${String(SEPA_START_DATE).trim()}T00:00:00.000Z`;
    const parsed = new Date(iso);
    if (Number.isNaN(parsed.getTime())) {
        throw new Error(`Invalid SEPA_START_DATE "${SEPA_START_DATE}" (expected format YYYY-MM-DD)`);
    }
    return parsed;
}
...
function filterInvoicesWithinWindow(invoices, windowStartMs) {
    return invoices.filter((inv) => {
        ...
        const invoiceDate = parseInvoiceDateValue(inv.invoice_date);
        ...
        const isInvoiceDateInWindow = invoiceDate ? invoiceDate.getTime() >= windowStartMs : false;
        return isCreatedInWindow || isModifiedInWindow || isInvoiceDateInWindow;
    });
}
...
async function pollInvoices() {
    ...
    // 🔹 Go-live cutoff: process only invoices on/after this date.
    const sepaStartDate = parseSepaStartDateOrThrow();
    const sepaStartDateMs = sepaStartDate.getTime();
    const sepaStartDateIso = sepaStartDate.toISOString();
    const filter = `(Created ge datetime'${sepaStartDateIso}' or Modified ge datetime'${sepaStartDateIso}' or InvoiceDate ge datetime'${sepaStartDateIso}')`;
    ...
    const recentInvoices = filterInvoicesWithinWindow(invoices, sepaStartDateMs);

    logger.info(`Fetched ${invoices.length} invoices`);
    logger.info(`Invoices on/after SEPA_START_DATE (${String(SEPA_START_DATE).trim()}): ${recentInvoices.length}`);
